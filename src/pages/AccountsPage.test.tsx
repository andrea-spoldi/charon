...
describe("AccountsPage isolated console session", () => {
  beforeEach(() => {
    invokedCommands = [];
  });

  it("calls open_aws_console_isolated with the account/role/session details when clicked", async () => {
    render(
      <AccountsPage
        ssoStatus={ssoStatus}
        accounts={[account]}
    ...
    const button = await screen.findByTitle(
      "Open in isolated session (keeps other accounts signed in)",
    );
    fireEvent.click(button);

    await waitFor(() =>
      expect(
        invokedCommands.some((c) => c.cmd === "open_aws_console_isolated"),
      ).toBe(true),
    );

    const call = invokedCommands.find(
      (c) => c.cmd === "open_aws_console_isolated",
    );
